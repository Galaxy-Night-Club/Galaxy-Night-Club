...
"auth/user-not-found": "Ce compte n'existe pas dans Firebase.",
"auth/wrong-password": "Le mot de passe est incorrect.",
"auth/too-many-requests": "Trop de tentatives. Attends un peu puis reessaie.",
"auth/network-request-failed": "Connexion reseau impossible pour joindre Firebase.",
"auth/unauthorized-domain": "Ce domaine n'est pas autorise dans Firebase Auth.",
...
if(!profile){
const fallbackProfile = getUserProfileFromEmail(email) || getUserProfileFromEmail(credential.user.email);
if(fallbackProfile){
profile = normalizeStaffProfile({
...fallbackProfile,
email: credential.user.email || fallbackProfile.email
});
const mergedUsers = [...getStoredUsers().filter((user)=> user.username !== profile.username), profile];
setUsers(mergedUsers);
} else {
...
loginMsg.className = "status-msg error";
loginMsg.innerText = getFirebaseStaffErrorMessage(error, "Connexion Firebase impossible. Verifie email, mot de passe ou Auth.");
}
}
